...
rule pretrain_rand_concept:
    input:
        ...
        concept=f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/randomized_data/{{concept}}"
    output:
    ...
    resources:
        ...
        queue="x86_1h",
    log:
        f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/logs/pretrain_rand_concept/{{concept}}/{{config_id}}"
    shell:
        "source scripts/setup_MLflow.sh && "
        "5_pretrain/scripts/pretrain_concept.py "
        "{input.cfg} {input.splits} {input.concept} "
        "{params.folder_name} {params.split_strategy} {params.run_type} "
