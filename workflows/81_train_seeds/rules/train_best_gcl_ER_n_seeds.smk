...
rule train_best_gcl_ER_n_seeds:
    input:
    ...
    output:
        expand(f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/checkpoints/graph_concept_learners/"+"best_gcl_ER_{{agg}}/{{config_id}}/test_conf_mat_from_best_val_{metric_name}.png", metric_name=config["follow_this_metrics"]),
        out_files=expand(f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/checkpoints/graph_concept_learners/"+"best_gcl_ER_{{agg}}/{{config_id}}/best_val_{metric_name}.pt", metric_name=config["follow_this_metrics"]),
    params:
        exclude=[
            "all_cells_contact",
            "all_cells_radius",
    ...
    resources:
        ...
        queue="x86_6h",
    log:
        f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/logs/train_gcl/best_gcl_ER_{{agg}}/{{config_id}}"
    shell:
