...
rule train_gcl:
    input:
        ...
        paths_to_pretrain_configs=f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/pretrain_results/best_model_per_concept/best_model_per_concept_{{er_status}}.yaml",
    ...
    output:
        expand(f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/checkpoints/graph_concept_learners/"+"{{er_status}}/{{config_id}}/test_conf_mat_from_best_val_{metric_name}.png", metric_name=config["follow_this_metrics"]),
        out_files=expand(f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/checkpoints/graph_concept_learners/"+"{{er_status}}/{{config_id}}/best_val_{metric_name}.pt", metric_name=config["follow_this_metrics"]),
    params:
        exclude=[
            "all_cells_contact",
            "all_cells_radius",
        ...
        run_type="train_{er_status}_gcl",
        randomize="False",
    resources:
        cores="1+1",
        mem="3G",
        queue="x86_6h",
    log:
        f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/{split_how}/logs/train_gcl/{{er_status}}/{{config_id}}"
    shell:
