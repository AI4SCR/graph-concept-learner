...
def get_all_graphs_and_datasets(wildcards):
    ...
    all_graphs = expand(f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/processed_data/{{concept}}/{{spl_id}}.pt", concept=CONTACT_CONCEPTS, spl_id=SMAPLE_IDS)
    all_datasets = expand(f"{root}/prediction_tasks/{prediction_target}/{normalized_with}/processed_data/{{concept}}", concept=RADIUS_CONCEPTS)
