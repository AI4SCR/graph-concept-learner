...
path_dupl = sys.argv[7]
path_miss = sys.argv[8]
path_unf = sys.argv[9]
...
# TODO: chage config
query = f"params.run_type = 'train_gcl' and params.folder_name = '{folder_name}' and params.split_strategy = '{split_strategy}'"
...
if len(df[df.duplicated(subset=["params.cfg_id"])]) > 0:
...
workflow_set = set(CFG_IDS)

# Get mlflow config concept combinations
mlflow_set = set(df["params.cfg_id"].values)
...
if len(missing_set) > 0:
    ...
    with open(path_miss, "w") as f:
        for concpet_cfg_id in missing_set:
            ...
            line = f"/dccstor/cpath_data/datasets/GCL/{dataset_name}/prediction_tasks/{pred_target}/{folder_name}/{split_strategy}/checkpoints/graph_concept_learners/{cfg_id}/{name_of_file}"
            f.write(f"{line}\n")
else:
    print(
